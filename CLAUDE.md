...
**Version**: 0.4.13 · **Status**: Phase 0–5 + PubMed/LLM-summary/MeSH + CSL citations + import/export + library utilities + review/security pass (73 integration checks green)
...
| `ui/{LibraryView,SearchView,RelatedView}.ts` | sidebar panes (`SearchView` holds the year/author/tag-chip filter state — pane-local, search only) |
...
npm test               # bundles test/integration.ts (obsidian shim) → live integration suite (73 checks)
```
