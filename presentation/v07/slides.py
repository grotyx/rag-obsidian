...
SLIDES = [
    ...
    dict(kind="cover", section="표지",
         ...
         say="안녕하세요. 오늘은 제가 만든 Obsidian 플러그인, Academic Paper Citation Manager를 "
             "소개합니다. 한 문장으로 말하면, 내 논문 서재가 직접 답하는 도구입니다."),

    # ── 1 · 문제 ─────────────────────────────────────────────────────────────
    dict(kind="section", section="문제", no="1", title="문제",
         say="먼저 문제부터 보겠습니다."),
    dict(kind="say", section="문제", text="논문,\n몇 편이나 읽으세요?",
         say="질문 하나 드리겠습니다. 한 주제로 논문을 몇 편이나 읽어보셨나요?"),
    ...
    dict(kind="say", section="문제", text="그래서\nAI에게 묻습니다",
         say="그래서 요즘은 AI에게 물어봅니다."),
    dict(kind="say", section="문제", text="답은\n술술 나옵니다",
         say="답은 정말 술술 나옵니다. 문장도 매끄럽고, 인용까지 달려 있습니다."),
    dict(kind="say", section="문제", text="그 논문,\n실제로 있나요?", hl="실제로",
         say="그런데 그 인용된 논문을 찾아보면 없는 경우가 있습니다. 저자도 저널도 그럴듯한데, "
             "존재하지 않는 논문입니다."),
    ...
    dict(kind="say", section="문제", text="내 서재는\n뜻으로 못 찾는다", hl="뜻",
         say="Zotero나 EndNote에 논문을 천 편 넣어두어도, 단어가 정확히 맞지 않으면 찾지 못합니다. "
             "MI로 검색하면 myocardial infarction이 나오지 않죠."),
    dict(kind="say", section="문제", text="쌓아둔 논문이\n일하지 않는다",
         say="결국 쌓아둔 논문이 저를 위해 일하지 않습니다."),
    ...
    dict(kind="say", section="아이디어", text="논문 1편 = 노트 1개",
         say="방법은 단순합니다. 논문 한 편이 Obsidian 노트 한 개가 됩니다."),
    dict(kind="note", section="아이디어",
         ...
         say="노트는 이렇게 생겼습니다. 맨 위에 저자, 연도, DOI, PMID가 표준 서지 형식으로 들어가고, "
             "아래에 AI가 쓴 구조화 요약과 제 메모가 붙습니다."),
    dict(kind="say", section="아이디어", text="그냥\n텍스트 파일입니다",
         say="특별한 데이터베이스가 아닙니다. 그냥 텍스트 파일, 마크다운입니다."),
    dict(kind="list", section="아이디어", items=["서버 없음", "계정 없음", "내 폴더에 그대로"],
         say="서버도, 계정도 필요 없습니다. 논문 정보는 전부 내 컴퓨터 폴더에 남습니다. "
             "프로그램을 지워도 파일은 그대로 남습니다."),
    dict(kind="say", section="아이디어", text="그 노트가\nAI 답의 근거", hl="근거",
         say="그리고 이 노트들이 그대로 AI가 답할 때의 근거가 됩니다."),
    ...
    dict(kind="steps", section="흐름", items=["DOI 붙여넣기", "노트 생성", "질문", "출처 달린 답"],
         say="DOI를 붙여넣으면 노트가 생기고, 질문하면 출처 달린 답이 나옵니다. 이게 전부입니다."),
    dict(kind="list", section="흐름", title="넣는 길",
         items=["DOI · PMID", "PubMed 검색", "PDF · Zotero 파일"],
         say="논문을 넣는 길은 여러 가지입니다. DOI나 PMID를 붙여넣거나, 플러그인 안에서 PubMed를 "
             "검색하거나, PDF를 넣거나, Zotero·EndNote에서 내보낸 파일을 그대로 가져옵니다."),
    dict(kind="say", section="흐름", text="넣으면\nAI가 요약합니다",
         say="넣으면 AI가 배경, 방법, 결과, 결론으로 나눠 요약합니다. 원문이 공개된 논문은 전문을, "
             "아니면 초록을 읽습니다."),
    dict(kind="say", section="흐름", text="주제 태그는\nMeSH로", hl="MeSH",
         say="주제 태그는 PubMed의 MeSH 용어로 자동으로 붙습니다. 그래서 Obsidian 그래프에서 "
             "주제별로 뭉쳐 보입니다."),
    dict(kind="say", section="흐름", text="검색은\n단어와 뜻을 함께", hl="뜻",
         say="검색은 두 가지를 동시에 봅니다. 정확한 단어, 그리고 뜻입니다."),
    ...
    dict(kind="cite", section="흐름",
         ...
         say="질문하면 답의 문장마다 번호가 붙습니다. 그 번호를 누르면 내 서재에 있는 논문의 실제 "
             "구절로 갑니다. 화면은 예시입니다."),
    ...
    dict(kind="say", section="흐름", text="저널 스타일은\n한 줄",
         say="참고문헌 형식은 저널 이름 한 줄로 바뀝니다. 만 개가 넘는 CSL 스타일을 자동으로 받아옵니다."),
    dict(kind="say", section="흐름", text="투고는\nWord로", hl="Word",
         say="그리고 투고용 Word 파일로 바로 내보냅니다."),
    dict(kind="say", section="흐름", text="놓친 논문도\n찾아줍니다", hl="놓친",
         say="마지막으로 인용 그래프입니다. 내 논문들이 여러 번 인용하는데 내 서재에는 없는 논문, "
             "즉 제가 놓친 핵심 논문을 찾아줍니다."),
    ...
    dict(kind="say", section="실전", text="스크리닝은\n키보드로 한 편씩",
         say="사람이 직접 스크리닝할 때는 한 편씩 초록을 보고, I는 포함, E는 제외, 키보드로 판정합니다."),
    dict(kind="say", section="실전", text="AI는 초안,\n결정은 사람", hl="사람",
         say="중요한 점은 이겁니다. AI가 만든 건 초안입니다. 포함 여부도, 권고도, 최종 결정은 패널이 합니다."),
    ...
    dict(kind="steps", section="AI 동료", items=["AI가 찾고 요약", "플러그인이 저장", "내가 확인"],
         say="AI가 PubMed를 찾고 요약을 쓰면, 저장은 플러그인이 안전하게 하고, 저는 결과를 확인합니다."),
    dict(kind="say", section="AI 동료", text="내 컴퓨터 안에서만",
         say="이 연결은 내 컴퓨터 안에서만 열리고, 접속 토큰으로 보호됩니다. 노트를 고칠 때는 그 사이 "
             "바뀌지 않았는지 먼저 확인합니다."),
    dict(kind="say", section="AI 동료", text="API 키 없이\n로그인으로", hl="로그인",
         say="ChatGPT의 Codex나 OpenCode에 이미 로그인해 있다면, 별도 API 키 없이 그 로그인으로 요약과 "
             "채팅을 돌릴 수 있습니다."),
    ...
    dict(kind="list", section="비교", items=["중복 병합", "철회 논문 표시", "무료 PDF 받기"],
         say="중복 논문을 합치고, 철회된 논문을 표시하고, 무료로 공개된 PDF를 한꺼번에 받습니다."),
    dict(kind="say", section="비교", text="그리고\n그 너머", hl="너머",
         say="그리고 그걸 넘어섭니다."),
    dict(kind="list", section="비교", items=["뜻으로 찾기", "출처 달린 답", "스크리닝 · PRISMA"],
         say="뜻으로 찾고, 출처가 달린 답을 받고, 스크리닝과 PRISMA까지 한 곳에서 합니다."),
    ...
    dict(kind="steps", section="시작", items=["Obsidian 설치", "플러그인 추가", "키 하나"],
         say="세 단계입니다. Obsidian을 설치하고, 플러그인을 추가하고, OpenRouter 키 하나를 넣으면 됩니다."),
    dict(kind="url", section="시작", url="github.com/grotyx/rag-obsidian", sub="무료 · MIT 라이선스",
         say="모든 코드는 여기 공개돼 있습니다. 무료이고, MIT 라이선스입니다."),
    ...
    dict(kind="qa", section="부록", q="인터넷 없이도 되나요?",
         ...
         say="노트와 검색 색인은 내 컴퓨터에 있습니다. AI 요약과 채팅은 제가 고른 제공자로 보내지고, "
             "Ollama를 쓰면 전부 로컬에서 돕니다."),
    dict(kind="qa", section="부록", q="내 데이터는 어디에 있나요?",
         a="vault 폴더의 마크다운 파일.\n서버·계정·사용 기록 전송 없음.",
         say="전부 제 vault 폴더의 마크다운 파일입니다. 플러그인에는 서버도, 계정도, 사용 기록 전송도 없습니다."),
